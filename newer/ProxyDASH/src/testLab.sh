...
sudo ip route add 130.136.37.0/24 dev eth0 src ${eth0} table dev1table
sudo ip route add default via 130.136.37.1 table dev1table

# 10.200.0.0/16 is the IP_network for wlan0 - 10.200.255.254 is the gateway
sudo ip route add 10.201.5.0/16 dev wlan0 src ${wlan0} table dev2table
sudo ip route add default via 10.201.255.254 table dev2table
