...
bool RequestManager::load_req(char *str_req, int size_str) {
	...
	if (strncmp(buff_req, "GET ", 4) == 0) {
		...
		debug_medium("HOST found: %s\n", host_name);
		debug_medium("PORT found: %d\n", server_port);
		debug_medium("PATH found: %s\n", path_name);

		switch(mpeg_dash){
		case MPEGDASH_FRAME:
			debug_medium("Type MPEGDASH_FRAME\n");
			break;
		case MPEGDASH_MANIFEST:
			debug_medium("Type MPEGDASH_MANIFEST\n");
			break;
		case MPEGDASH_INIT:
			debug_medium("Type MPEGDASH_INIT\n");
			break;
		case MPEGDASH_NO_TYPE:
			debug_medium("Type UNKNOWN\n");
			break;
		}
